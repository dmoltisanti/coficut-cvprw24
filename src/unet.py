...
# DM cloned from https://github.com/mberkay0/pretrained-backbones-unet/tree/main
...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        mlp_channels_in = 2048
        self.sigmoid = nn.Sigmoid()
        self.mlp = nn.Sequential(*build_mlp(mlp_channels_in, 1, hidden_units=hidden_units, bn=False,
                                            dropout=backbone_kwargs['drop_rate']))

    def forward(self, x: torch.Tensor, return_bottleneck_features=True, encoder_only=False,
                only_features=False):
        ...
        if only_features:
            return f[0].mean(dim=(2, 3))  # list is reversed, so f[0] contains the last layer embedding

        ratio_output = self.sigmoid(self.mlp(f[0].mean(dim=(2, 3))))  # DM
        ...
        if return_bottleneck_features:
            return x, f[0], ratio_output  # list is reversed, so f[0] contains the last layer embedding
        else:
            return x, ratio_output
